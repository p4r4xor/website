...
On the other hand, we know a lot more if the host has already been crawled to some extent. Say we've never seen a particular Crunchbase URL but have crawled thousands of other `/company/*` pages == crawler has some idea of what that path usually contains, how often those pages are useful, what they cost to fetch and whether they tend to produce more useful links.
...
A fairly simple starting point would be to learn refresh cadence at the path level. A new Crunchbase `/company/*` page inherits what we know about similar company pages; if it keeps changing meaningfully, pull its next crawl closer and if it remains unchanged across multiple crawls, keep pushing it out.
...
I kinda wanted to cover adaptive politeness, diagnosing why requests fail and using that feedback to choose the right rate/resources for the next request but this blog is already prose heavy and will likely move it to the next one.

This blog also doesn't evaluate whether the scheduling ideas above actually work. I'm aiming to cover that next by simulating a few policies and comparing what they crawl, what they miss and how much they cost.

### Closing notes

> Not everyone can build a good crawler.

I've been working in this space for some time now and useful data has only become harder to access reliably. People who can acquire it well and turn it into something useful will make bank (+1).
